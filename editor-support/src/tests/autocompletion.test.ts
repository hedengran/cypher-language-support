...
describe('MATCH auto-completion', () => {
  test('Correctly completes MATCH', async () => {
    const query = 'M';
    const position = createPosition(0, query.length);
  ...
  test('Correctly completes procedure name in CALL', async () => {
    ...
    await testCompletionContains(
      ...
      new MockDbInfo(
        ...
        [],
        new Map([
          ['foo.bar', createSignatureInformation('')],
          ['dbms.info', createSignatureInformation('')],
          ['somethingElse', createSignatureInformation('')],
          ['xx.yy', createSignatureInformation('')],
          ['db.info', createSignatureInformation('')],
        ]),
      ),
      [
...
describe('Functions auto-completion', () => {
  const functionSignatures: [string, SignatureInformation][] = [
    ['a.b', createSignatureInformation('')],
    ['xx.yy.proc', createSignatureInformation('')],
    ['xx.yy.procedure', createSignatureInformation('')],
    ['db.info', createSignatureInformation('')],
  ];
